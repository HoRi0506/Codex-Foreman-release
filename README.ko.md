...
> 사전 릴리즈 안내: `0.0.5-pre`는 아직 사전 릴리즈입니다. macOS가 우선 검증 경로입니다. Linux와 Windows 설치/실행 표면도 포함되어 있지만, 환경에 따라 아직 정상 동작하지 않을 수 있습니다.
...
소스 repo는 captain-constrained MCP control plane, no-mutation preflight, 그리고 `$cap` skill을 bootstrap 안내 쪽으로 줄이는 방향으로 `0.0.5-pre`를 준비 중입니다. 해당 pre-release runtime, docs, validation이 public release 준비 상태가 될 때까지 이 release repo는 `0.0.5-pre` 기준으로 유지합니다.

pre-release card 본문은 이제 `docs/release/notes/v0.0.5.md`에 있습니다. public release tree는 `share/skills/cap/SKILL.md`를 얇은 placeholder로 유지하고, `scripts/build-release-asset.sh`가 installable asset을 만들 때 source repo의 authoritative skill로 교체하므로 release repo를 둘러봐도 installable skill 전체가 노출되지 않습니다.

release-repo history rewrite 여부는 명시적인 release-operator 결정이 있을 때까지 deferred 상태로 둡니다.
