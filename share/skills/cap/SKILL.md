...
- do not make `mcp__codex_foreman__foreman_auto_entry` a required front door for ordinary `$cap` work
- use `mcp__codex_foreman__foreman_auto_entry` only for compatibility checks, diagnostics, or when the operator explicitly asks to test the automatic entry surface
...
- only allow `captain` direct mutation when Foreman cannot launch the specialist path honestly, bounded reclaim and one explicit captain retry already failed, or the operator explicitly approves a host-local fallback
...
- if status shows `reclaim_plan.reclaim_needed_worker_count > 0` or a worker timeout/reclaim boundary, call the bounded Foreman reclaim path before considering host-local fallback
...
2. For a fresh request, use `mcp__codex_foreman__foreman_start` as the default public entry.
3. For the ordinary public loop, start with a captain-first planning run:
   - set the scope to one bounded planning checkpoint
   - set `task_kind=plan` when the next specialist is not yet obvious
   - use the smallest truthful bounded scope instead of an unbounded chain drain
4. After start, read `mcp__codex_foreman__foreman_status` before deciding the next move.
5. Launch at most one bounded specialist step at a time with `mcp__codex_foreman__foreman_orchestrate`.
6. After each specialist result, return to a captain checkpoint before materializing the next specialist.
7. When captain needs a different next specialist in the same run, call `mcp__codex_foreman__foreman_orchestrate` with:
   ...
   - optional `resolve_summary`: a short captain summary for the next checklist item title
8. After that replan call, read `mcp__codex_foreman__foreman_status` again so the captain checkpoint is visible, then run another bounded `foreman_orchestrate` call to launch the selected specialist.
...
11. If a worker stalls, let Foreman reclaim it first. Only after reclaim and an explicit captain retry/reassignment fail should the host consider a visible degraded fallback.
12. To close the loop honestly, have captain call `mcp__codex_foreman__foreman_orchestrate` with `resolve_outcome` and `resolve_summary` when acceptance is met, blocked, failed, or cancelled.
13. Reply to the operator only when the request is complete, explicitly blocked, or waiting on a real operator decision.
...
- `foreman_auto_entry` still exists for compatibility and diagnostics, but it is no longer the default public `$cap` entry path for `0.0.1`
